...
function run {
    ...
    if [[ "$CHANN" == "" || "$CHANN" == "$1" ]]; then
        FILES=$(ls -l [0-9]*/landsCombine${NAM}_${WHAT}.*.root 2> /dev/null | awk '{if ($5 > 1000) print}' | wc -l);
        if [[ "$FILES" == "0" ]]; then return; fi;
        if [[ "$VERB" == "1" ]]; then 
            ./hadd2 -f results/landsCombine${NAM}_${WHAT}.root [0-9]*/landsCombine${NAM}_${WHAT}.*.root
        else
            ./hadd2 -f results/landsCombine${NAM}_${WHAT}.root [0-9]*/landsCombine${NAM}_${WHAT}.*.root > /dev/null 2>&1
        fi;
        echo "landsCombine${NAM}_${WHAT}.root   ($FILES files)"
    fi;
}
