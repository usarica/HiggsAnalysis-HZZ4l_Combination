...
class datacardClass_HCTau:
    ...
    def makeCardsWorkspaces(self, theMH, theOutputDir, theInputs,theTemplateDir="templates2D",theDataAppendDir=""):
        ...
        D1Name = "CMS_zz4l_TPerp"
        D2Name = "CMS_zz4l_smd"

        D1 = ROOT.RooRealVar(D1Name,D1Name,0,5000)
        D1.setBins(50)
        D2 = ROOT.RooRealVar(D2Name,D2Name,0,1)
        D2.setBins(50)
	...
	# n, alpha are left side parameters of DoubleCB
        ...
        signalHistFuncs_ScaleResUp = ROOT.RooArgList()
        signalHistFuncs_ScaleResDown = ROOT.RooArgList()

        for tt in range(0,nctau) :
          val_ctau = (x_max-x_min) / (nctau-1) * tt + x_min
          signalTemplates = "{0}_templates_Modified_Nominal_ScaleResUpDown_CTau{1:.0f}.root".format(self.appendName,val_ctau)
          templateSigName = "{0}/{1}".format(mytemplateDir,signalTemplates)
          sigTempFile = ROOT.TFile(templateSigName)
          Sig_T = sigTempFile.Get("T_2D")
          Sig_T.SetName("T_ZZ_{0:.0f}_{1}_2D_{2:.0f}".format(self.sqrts,self.appendName,val_ctau))
          Sig_T_ScaleResUp = sigTempFile.Get("T_2D_ScaleResUp")
          Sig_T_ScaleResUp.SetName("T_ZZ_{0:.0f}_{1}_2D_{2:.0f}_ScaleResUp".format(self.sqrts,self.appendName,val_ctau))
          Sig_T_ScaleResDown = sigTempFile.Get("T_2D_ScaleResDown")
          Sig_T_ScaleResDown.SetName("T_ZZ_{0:.0f}_{1}_2D_{2:.0f}_ScaleResDown".format(self.sqrts,self.appendName,val_ctau))

          if t==0:
            dBinsX = Sig_T.GetXaxis().GetNbins()
            dLowX = Sig_T.GetXaxis().GetXmin()
            dHighX = Sig_T.GetXaxis().GetXmax()
            dBinsY = Sig_T.GetYaxis().GetNbins()
            dLowY = Sig_T.GetYaxis().GetXmin()
            dHighY = Sig_T.GetYaxis().GetXmax()
...
#            r_fai_norm = ROOT.RooFormulaVar("ggH_norm","ggH_norm","(abs(@3)+abs(@4))>1 ? 0. : TMath::Max((@0+@1+@2)*(1-abs(@5)),0)",RooArgList(r_fai_pures_norm,r_fai_realints_norm,r_fai_imagints_norm,x,y,alpha_zz4l))
          ...
          Sig_T_ScaleResUp_hist = ROOT.RooDataHist ("T_ScaleResUp_hist_{0:.0f}_{1:.0f}_{2:.0f}".format(self.channel,self.sqrts,val_ctau),"", ROOT.RooArgList(D1,D2),Sig_T_ScaleResUp)
          Sig_T_ScaleResDown_hist = ROOT.RooDataHist ("T_ScaleResDown_hist_{0:.0f}_{1:.0f}_{2:.0f}".format(self.channel,self.sqrts,val_ctau),"", ROOT.RooArgList(D1,D2),Sig_T_ScaleResDown)
          ...
          Sig_T_ScaleResUp_histfunc = ROOT.RooHistFunc ("T_ScaleResUp_histfunc_{0:.0f}_{1:.0f}_{2:.0f}".format(self.channel,self.sqrts,val_ctau),"", ROOT.RooArgSet(D1,D2),Sig_T_ScaleResUp_hist)
          Sig_T_ScaleResDown_histfunc = ROOT.RooHistFunc ("T_ScaleResDown_histfunc_{0:.0f}_{1:.0f}_{2:.0f}".format(self.channel,self.sqrts,val_ctau),"", ROOT.RooArgSet(D1,D2),Sig_T_ScaleResDown_hist)

          signalHistFuncs_Nominal.add(Sig_T_histfunc)
          signalHistFuncs_ScaleResUp.add(Sig_T_ScaleResUp_histfunc)
          signalHistFuncs_ScaleResDown.add(Sig_T_ScaleResDown_histfunc)



        ggHpdfName = "ggH_RooSpinZeroPdf_{0:.0f}_{1:.0f}".format(self.channel,self.sqrts)
        ggHpdf = ROOT.HZZ4L_RooCTauPdf(ggHpdfName,ggHpdfName,D1,D2,x,signalHistFuncs_Nominal,x_min,x_max)
   

        ggHpdfName_syst1Up = "ggH_RooSpinZeroPdf_ScaleResUp_{0:.0f}_{1:.0f}".format(self.channel,self.sqrts)
        ggHpdf_syst1Up = ROOT.HZZ4L_RooSpinZeroPdf_2D(ggHpdfName_syst1Up,ggHpdfName_syst1Up,D1,D2,x,signalHistFuncs_ScaleResUp,x_min,x_max)
        ggHpdfName_syst1Down = "ggH_RooSpinZeroPdf_ScaleResDown_{0:.0f}_{1:.0f}".format(self.channel,self.sqrts)
        ggHpdf_syst1Down = ROOT.HZZ4L_RooSpinZeroPdf_2D(ggHpdfName_syst1Down,ggHpdfName_syst1Down,D1,D2,x,signalHistFuncs_ScaleResDown,x_min,x_max)
...
#        qqZZTemplate.Scale(1.0/qqZZTemplate.Integral())
        ...
        qqZZTempDataHist = ROOT.RooDataHist(TemplateName,TemplateName,ROOT.RooArgList(D1,D2),qqZZTemplate)
        PdfName = "qqZZ_TemplatePdf_{0:.0f}_{1:.0f}".format(self.channel,self.sqrts)
        qqZZTemplatePdf = ROOT.RooHistPdf(PdfName,PdfName,ROOT.RooArgSet(D1,D2),qqZZTempDataHist)

        ggZZTemplate = bkgTempFile.Get("template_ggZZ")
        
        TemplateName = "ggZZTempDataHist_{0:.0f}_{1:.0f}".format(self.channel,self.sqrts)
        ggZZTempDataHist = ROOT.RooDataHist(TemplateName,TemplateName,ROOT.RooArgList(D1,D2),ggZZTemplate)
        PdfName = "ggZZ_TemplatePdf_{0:.0f}_{1:.0f}".format(self.channel,self.sqrts)
        ggZZTemplatePdf = ROOT.RooHistPdf(PdfName,PdfName,ROOT.RooArgSet(D1,D2),ggZZTempDataHist)
...
#        ZjetsTemplate.Scale(1.0/ZjetsTemplate.Integral())
        ...
        ZjetsTempDataHist = ROOT.RooDataHist(TemplateName,TemplateName,ROOT.RooArgList(D1,D2),ZjetsTemplate)
        PdfName = "Zjets_TemplatePdf_{0:.0f}_{1:.0f}".format(self.channel,self.sqrts)
        ZjetsTemplatePdf = ROOT.RooHistPdf(PdfName,PdfName,ROOT.RooArgSet(D1,D2),ZjetsTempDataHist)
...
#        ZjetsTemplateDown.Scale(1.0/ZjetsTemplateDown.Integral())
        ...
        ZjetsTempDataHistDown = ROOT.RooDataHist(TemplateName,TemplateName,ROOT.RooArgList(D1,D2),ZjetsTemplateDown)
        PdfName = "Zjets_TemplateDownPdf_{0:.0f}_{1:.0f}".format(self.channel,self.sqrts)
        ZjetsTemplatePdfDown = ROOT.RooHistPdf(PdfName,PdfName,ROOT.RooArgSet(D1,D2),ZjetsTempDataHistDown)
        ...
        ZjetsTempDataHistUp = ROOT.RooDataHist(TemplateName,TemplateName,ROOT.RooArgList(D1,D2),ZjetsTemplateUp)
        PdfName = "Zjets_TemplateUpPdf_{0:.0f}_{1:.0f}".format(self.channel,self.sqrts)
        ZjetsTemplatePdfUp = ROOT.RooHistPdf(PdfName,PdfName,ROOT.RooArgSet(D1,D2),ZjetsTempDataHistUp)
        ...
        ZjetsTemplateMorphPdf = ROOT.FastVerticalInterpHistPdf2D(MorphName,MorphName,D1,D2,False,funcList_zjets,morphVarListBkg,1.0,1)
