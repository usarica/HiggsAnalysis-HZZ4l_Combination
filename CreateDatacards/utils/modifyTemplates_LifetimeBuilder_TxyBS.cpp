...
void symmetrize_PromptTemplates(TH1F* hrepair, int iProd);

void regularizeCTauSlice(TH1F* hSlice, int iProd, bool postProcessing = false);
...
void modifySigTemplates(TString dir, int erg_tev, int folder){
  TString sqrts = Form("%iTeV", erg_tev);
  TString channame = "2e2mu";
  if (folder==0) channame = "4mu";
  else if (folder==1) channame = "4e";
  ...
  TH1F* h_KDarray_Unmodified[nCTau][nProd*kNumTxySysts];
  ...
  for (int iProd=0; iProd<nProd; iProd++){
    for (int ct = 0; ct < nCTau; ct++){
      ...
      for (int ss = 0; ss < kNumTxySysts; ss++){
        ...
        gROOT->cd();
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss] = (TH1F*)h_1D[ss]->Clone(Form("%s_Prod%i_CTau%i_Unmodified", h_1D[ss]->GetName(), iProd, genctau));
        finput->cd();
        if (ss>0) h_1D[ss]->Scale(h_1D[0]->Integral()/h_1D[ss]->Integral());
        if (genctau==0) symmetrize_PromptTemplates(h_1D[ss], iProd);
        if(genctau>500) regularizeCTauSlice(h_1D[ss],iProd);
        if (ss==0 && ct==0){
      ...
      for (int ss = 0; ss < kNumTxySysts; ss++){
        gROOT->cd();
        h_KDarray[ct][kNumTxySysts*iProd + ss] = (TH1F*)h_1D[ss]->Clone(Form("%s_Prod%i_CTau%i", h_1D[ss]->GetName(), iProd, genctau));
        finput->cd();
        delete h_1D[ss];
        delete h_2D[ss];
      }
    ...
    TString plotDir = dir + "/ValidationPlots/Smoothening_Along_CTau/" + sqrts + "/" + channame + "/";
    if (iProd==0) plotDir.Append("ggH");
    else plotDir.Append(cProduction[iProd]);
    plotDir.Append("/");
  ...
  for (int ct = 0; ct < nCTau; ct++){
    ...
    // Regularize systematics again
    for (int iProd=0; iProd<nProd; iProd++){
      for (int ss=1; ss<kNumTxySysts; ss++){
        if (genctau>500) regularizeCTauSlice(h_KDarray[ct][kNumTxySysts*iProd + ss], iProd, true);
      }
    }
    ...
    for (int iProd=0; iProd<nProd; iProd++){
      for (int ss = 0; ss < kNumTxySysts; ss++){
        ...
        if (iProd>0){
          ...
          h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->SetName(Form("%s_Unmodified", h_KDarray[ct][kNumTxySysts*iProd + ss]->GetName()));
          h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->Scale(h_KDarray[ct][kNumTxySysts*iProd + ss]->Integral()/h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->Integral());
          foutput->WriteTObject(h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]);

          gROOT->cd();
          delete h_KDarray[ct][kNumTxySysts*iProd + ss];
          delete h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss];
        }
        else{
          h_KDarray[ct][kNumTxySysts*iProd + ss]->SetName(hname);
          ...
          h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->SetName(Form("%s_Unmodified", hggH->GetName()));
          h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->Scale(hggH->Integral()/h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->Integral());
          foutput->WriteTObject(h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]);
          delete hggH;
          delete h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss];
...
void modifyBkgTemplates(TString dir, int erg_tev, int folder){
  TString sqrts = Form("%iTeV", erg_tev);
  TString channame = "2e2mu";
  if (folder==0) channame = "4mu";
  else if (folder==1) channame = "4e";
...
//	TString cinput_TxyDown = dir + "/" + sqrts + "/" + channame + "_templates_Nominal_bkg.root";
  ...
  for (int ii = 0; ii < TxySyst; ii++){
    ...
    symmetrize_PromptTemplates(ggzz_1d_txy[ii], -1);
    symmetrize_PromptTemplates(qqzz_1d_txy[ii], -1);
...
void plotSigTemplates(TString dir, int erg_tev, int folder){
  TString sqrts = Form("%iTeV", erg_tev);
  TString channame = "2e2mu";
  if (folder==0) channame = "4mu";
  else if (folder==1) channame = "4e";
  TString chantitle = "2e+2#mu";
  if (folder==0) chantitle = "4#mu";
  else if (folder==1) chantitle = "4e";

  const int nProd = 5;
  const int kNumTxySysts=3;
  TString cProduction[nProd]={ "ggH", "VBFH", "WH", "ZH", "ttH" };
  TString strTxySystAppend[kNumTxySysts]={ "TxyNominal", "TxyUp", "TxyDown" };
  ...
  TString cinput_Txymain = dir + "/" + sqrts + "/" + channame + "_templates_";
  TString plotDir_main = dir + "/ValidationPlots/Smoothening_Final/" + sqrts + "/" + channame + "/";

  for (int ct = 0; ct < nCTau; ct++){
    int genctau = ct*maxCTau / (nCTau - 1);
    TString cinput = cinput_Txymain;
    cinput.Append(Form("%i%s", genctau, "_Modified.root"));
    TFile* finput = new TFile(cinput, "read");
    for (int iProd=0; iProd<nProd; iProd++){
      for (int ss = 0; ss < kNumTxySysts; ss++){
        TString hname = "T_2D_";
        ...
        gROOT->cd();
        h_KDarray[ct][kNumTxySysts*iProd + ss] = (TH1F*)htemp->Clone(Form("%s_PlotClone", htemp->GetName()));
        finput->cd();
        delete htemp;

        hname.Append("_Unmodified");
        htemp = (TH1F*)finput->Get(hname);
        htemp->Scale(1./htemp->Integral());
        gROOT->cd();
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss] = (TH1F*)htemp->Clone(Form("%s_PlotClone", htemp->GetName()));
        finput->cd();
        delete htemp;
      }
    }
  ...
  for (int iProd=0; iProd<nProd; iProd++){
    TString plotDir = plotDir_main;
    plotDir.Append(cProduction[iProd]);
    plotDir.Append("/");
    TString mkdirCommand = "mkdir -p ";
    mkdirCommand.Append(plotDir);
    gSystem->Exec(mkdirCommand);
    ...
    for (int ct = 0; ct < nCTau; ct++){
      ...
      TString canvasname_2D = "cCanvas_";
      canvasname_2D.Append(Form("%s_", cProduction[iProd].Data()));
      if (genctau<10) canvasname_2D.Append(Form("CTau000%i_", genctau));
      else if (genctau<100) canvasname_2D.Append(Form("CTau00%i_", genctau));
      else if (genctau<1000) canvasname_2D.Append(Form("CTau0%i_", genctau));
      else if (genctau<10000) canvasname_2D.Append(Form("CTau%i_", genctau));
      canvasname_2D = canvasname_2D + channame + "_" + sqrts + "_SignalTxyBS_FinalTemplatesComparison";
      TCanvas* c2D = new TCanvas(canvasname_2D, "", 8, 30, 800, 800);
      c2D->cd();
      gStyle->SetOptStat(0);
      c2D->SetFillColor(0);
      ...
      double max_plot = 0;
      for (int ss=0; ss<3; ss++){
        for (int binx=0; binx<=h_KDarray[ct][kNumTxySysts*iProd + ss]->GetNbinsX(); binx++) h_KDarray[ct][kNumTxySysts*iProd + ss]->SetBinError(binx, 0);
        h_KDarray[ct][kNumTxySysts*iProd + ss]->SetLineWidth(2);
        if (ss==0) h_KDarray[ct][kNumTxySysts*iProd + ss]->SetLineColor(kBlack);
        else if (ss==1) h_KDarray[ct][kNumTxySysts*iProd + ss]->SetLineColor(kRed);
        else if (ss==2) h_KDarray[ct][kNumTxySysts*iProd + ss]->SetLineColor(kBlue);
        h_KDarray[ct][kNumTxySysts*iProd + ss]->SetMarkerColor(h_KDarray[ct][kNumTxySysts*iProd + ss]->GetLineColor());
        h_KDarray[ct][kNumTxySysts*iProd + ss]->SetTitle("");
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetTitle("tanh(T_{xy}^{Beam} / 1250 #mum)");
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetTitle(Form("Rate / %.2f", h_KDarray[ct][kNumTxySysts*iProd + ss]->GetXaxis()->GetBinWidth(1)));
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetNdivisions(505);
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetLabelFont(42);
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetLabelOffset(0.007);
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetLabelSize(0.04);
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetTitleSize(0.06);
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetTitleOffset(0.9);
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetTitleFont(42);
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetNdivisions(505);
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetLabelFont(42);
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetLabelOffset(0.007);
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetLabelSize(0.04);
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetTitleSize(0.06);
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetTitleOffset(1.2);
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetTitleFont(42);

        for (int binx=0; binx<=h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetNbinsX(); binx++) h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->SetBinError(binx, 0);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->SetLineWidth(2);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->SetMarkerStyle(31);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->SetMarkerSize(1.2);
        if (ss==0) h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->SetLineColor(kBlack);
        else if (ss==1) h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->SetLineColor(kRed);
        else if (ss==2) h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->SetLineColor(kBlue);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->SetMarkerColor(h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetLineColor());
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->SetTitle("");
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetTitle("tanh(T_{xy}^{Beam} / 1250 #mum)");
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetTitle(Form("Rate / %.2f", h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetXaxis()->GetBinWidth(1)));
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetNdivisions(505);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetLabelFont(42);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetLabelOffset(0.007);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetLabelSize(0.04);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetTitleSize(0.06);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetTitleOffset(0.9);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetXaxis()->SetTitleFont(42);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetNdivisions(505);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetLabelFont(42);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetLabelOffset(0.007);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetLabelSize(0.04);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetTitleSize(0.06);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetTitleOffset(1.2);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetTitleFont(42);

        max_plot = max(max(max_plot, h_KDarray[ct][kNumTxySysts*iProd + ss]->GetMaximum()), h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetMaximum());
      }
      for (int ss=0; ss<3; ss++){
        h_KDarray[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetRangeUser(0, max_plot*1.3);
        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->GetYaxis()->SetRangeUser(0, max_plot*1.3);
      }

      l2D->AddEntry(h_KDarray[ct][kNumTxySysts*iProd + 0], "Nominal", "l");
//      l2D->AddEntry(h_KDarray_Unmodified[ct][kNumTxySysts*iProd + 0], "Nominal (initial)", "p");
      l2D->AddEntry(h_KDarray[ct][kNumTxySysts*iProd + 1], "Res. up", "l");
      l2D->AddEntry(h_KDarray[ct][kNumTxySysts*iProd + 2], "Res. down", "l");

      for (int ss=0; ss<3; ss++){
        if (ss==0) h_KDarray[ct][kNumTxySysts*iProd + ss]->Draw("hist");
        else h_KDarray[ct][kNumTxySysts*iProd + ss]->Draw("histsame");
//        h_KDarray_Unmodified[ct][kNumTxySysts*iProd + ss]->Draw("x0psame");
      }
      l2D->Draw("same");
      pt->Draw();

      TPaveText* pt20;
      pt20 = new TPaveText(0.66, 0.88, 0.90, 0.92, "brNDC");
      pt20->SetBorderSize(0);
      pt20->SetTextAlign(12);
      pt20->SetTextSize(0.03);
      pt20->SetFillStyle(0);
      pt20->SetTextFont(42);
      TText* text20 = pt20->AddText(0.01, 0.01, chantitle);
      pt20->Draw();

      TPaveText *pt10 = new TPaveText(0.66, 0.82, 0.90, 0.88, "brNDC");
      pt10->SetBorderSize(0);
      pt10->SetTextAlign(12);
      pt10->SetTextSize(0.03);
      pt10->SetFillStyle(0);
      pt10->SetTextFont(42);
      TText* text10 = pt10->AddText(0.01, 0.01, Form("c#tau_{H} = %i #mum", genctau));
      pt10->Draw();
...
void modifyTemplates_LifetimeBuilder(TString dir, int processSig=1, int processBkg=1){
	for (int e = 7; e <= 8; e++){
		for (int f = 0; f < 3; f++){
			if(processBkg==1) modifyBkgTemplates(dir, e, f);
      if (processSig==1){
        modifySigTemplates(dir, e, f);
        plotSigTemplates(dir, e, f);
      }
    }
	}
}
...
void regularizeCTauSlice(TH1F* hSlice, int iProd, bool postProcessing){
  ...
  if (iProd==0) bin_midright+=3;
  else if (iProd!=4) bin_midright+=2;
  else if (iProd==4) bin_midright+=1;
  bin_midleft = bin_mid-5;
  int bin_first = 3, bin_last = nbins_slice - 4;

  double* xx_second;
  double* yy_second;

  int nIter = 1000;
  if (postProcessing) nIter = 50;
  for (int it=0; it<nIter; it++){
    double threshold = 0.003;
    if (postProcessing) threshold *= 10.;
    for (int binIt = bin_midright; binIt<=bin_last; binIt++){
      int ctr = 0;
      int nbins_second = nbins_slice - bin_mid;
      ...
      double center = hSlice->GetXaxis()->GetBinCenter(binIt);
      double val = spline->Eval(center);
      if (iProd==0 && center>0.4 && center<0.8 && !postProcessing) threshold /= 3.;
      if (fabs(hSlice->GetBinContent(binIt)-val)>threshold*val){
//        cout << "ALERT: Replacing " << hSlice->GetBinContent(binIt) << " with " << val << endl;
        hSlice->SetBinContent(binIt, val);
      }
...
void symmetrize_PromptTemplates(TH1F* hrepair, int iProd){
  ...
  if (iProd==4){
    for (int bin=right_minbin; bin<=right_maxbin; bin++){
      int corrBin = left_maxbin - (bin-right_minbin);
      hrepair->SetBinContent(bin, hrepair->GetBinContent(corrBin));
      hrepair->SetBinError(bin, hrepair->GetBinError(corrBin));
    }
  }
  else{
    for (int bin=right_minbin; bin<=right_maxbin; bin++){
