...
void modifySigTemplates(TString dir, TString sqrts = "7TeV", TString channame = "2e2mu"){
	...
	for (int f = 0; f < kNumSysts; f++){
		...
		for (int t = 0; t < kNumTemplates; t++){
			...
			if(channame.CompareTo("2e2mu")==0 && t==6) hsig[kNumTemplates * f + t]->Add(hsig[kNumTemplates * f + t],-1);
...
void modifyTemplates_fL1Builder(TString dir, int processSig=1, int processBkg=1){
